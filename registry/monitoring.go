...
func (m *instrumentedRegistry) GetSortedRepositoryImages(id image.Name, lessfn image.SortLessFunc) (res []image.Info, err error) {
	start := time.Now()
	res, err = m.next.GetSortedRepositoryImages(id, lessfn)
